package com.libs.utils.permission;
...
import com.libs.utils.permission.handle.Handle;
import com.libs.utils.permission.view.Request;
 ...
 */
public abstract class PermissionManage {
     ...
     */
    public PermissionManage(@NonNull Activity activity, @NonNull String[] permissions, String defaultTips) {
     ...
     */
    public PermissionManage(@NonNull Activity activity, @NonNull String... permissions) {
     ...
     * @param defaultTips 为没有重写{@link #onTips}时的默认值
     */
    public PermissionManage(@Nullable Activity activity, @NonNull Context context, @NonNull String[] permissions, String defaultTips) {
     ...
     * 只当activity可能传入null 或 为null时（仅仅检测权限） 才可以选择使用本方法，因为activity存在时context参数不会生效，等价于没有context参数的构造函数。
     */
    public PermissionManage(@Nullable Activity activity, @NonNull Context context, @NonNull String... permissions) {
     ...
     */
    protected void onTipsDialogView(String tips, boolean isCancel, int viewTipsCount, ArrayList<String> permissions,
                                    boolean isFinal, final Runnable okCall, final Runnable cancelCall) {
    ...
    private void _queryAll(ArrayList<String> checks, HashMap<String, Handle.CheckResult> handleRequests, ArrayList<String> finalList, ArrayList<String> noGrant) {
        for (String item : checks) {
            ...
            if (pItem != null && pItem.Handle != null) {
                Handle.CheckResult res = pItem.Handle.Check(PermissionManage.this, item);
                if (!res.IsResolve) {
                    //无权限
        ...
        __execTipsWrap(false, notEmpty, baseCount + (res.IsFinalReject ? 1 : 0), permissions, res.IsFinalReject,
                new Runnable() {
                    ...
                    public void run() {
                        ...
                        pItem.Handle.Request(PermissionManage.this, permission, new Handle.RequestCallback() {
                            @Override
