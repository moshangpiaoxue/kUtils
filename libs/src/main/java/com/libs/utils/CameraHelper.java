...
public class CameraHelper {
    ...
    private int mCameraFacing = CameraCharacteristics.LENS_FACING_BACK;                //默认使用前置摄像头;
    ...
    private OnTakePic onTakePic;

    public interface OnTakePic {
        void getTakeResult(boolean isSave, File file, String path, Bitmap bitmap);
    }


    public CameraHelper(Activity activity, TextureView textureView, OnTakePic onTakePic) {
        this.mActivity = activity;
        this.mTextureView = textureView;
        this.onTakePic = onTakePic;
            ...
            mImageReader.setOnImageAvailableListener(new ImageReader.OnImageAvailableListener() {
                ...
                public void onImageAvailable(ImageReader reader) {
                    ...
                    if (mCameraSensorOrientation == 270) {
                        bitmap = BitmapUtil.getBitmapForMirror(bitmap);
                    }
                    final String path = mActivity.getExternalCacheDir().getAbsolutePath() + File.separator;

                    if (mCameraFacing == CameraCharacteristics.LENS_FACING_FRONT) {
                        bitmap = BitmapUtil.getBitmapForRotate(bitmap, 360 - mCameraSensorOrientation);
                    } else {
                        bitmap = BitmapUtil.getBitmapForRotate(bitmap, mCameraSensorOrientation);
                    }
                    bitmap = BitmapUtil.getBitmapForZoom(bitmap, 600);
                    final String name = DateUtil.getMS() + ".jpg";
                    File file = new File(path);
                    final File targetFile = new File(path + name);
                    image.close();
                    final boolean result = BitmapUtil.saveBitmap(bitmap, file, targetFile);
                    //注意这里是异步线程，不允许直接操作界面
                    if (result) {
                        LogUtil.i( "保存成功，路径 : " + path + name);
                    } else {
                        LogUtil.i( "保存失败");
                    }
                    if (onTakePic!=null){
                        ...
                        mActivity.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                onTakePic.getTakeResult(result,targetFile,path+ name, finalBitmap);
                            }
                        });
                    }
                }
